...
date: "First posted: 3 January 2019"
output:
  html_document:
    highlight: zenburn
    toc: yes
...
pages_plot <- ggplot(rdata, aes(date, pages, color = title, group = title)) +
...
```{r}
# Weekly trend ----
weekly_rdata <- rdata_completed %>%
  group_by(year, week) %>%
  summarise(psum = sum(pages), 
            days = n(),
            m = mean(pages),
            sd = sd(pages)
            ) %>%
...
ggplot(weekly_rdata %>% filter(year == 2019), aes(as.factor(week), m)) +
  geom_point(data = rdata_completed %>% filter(year == 2019), 
             aes(y = pages),
             shape = 1, 
             #alpha = 2/3,
             position = pj,
...
# Summary data
weekday_rdata_sum <- weekday_rdata %>% 
...
ggplot(weekday_rdata_sum %>% filter(year == 2019), aes(weekday, m)) +
  geom_point(data = weekday_rdata %>% filter(year == 2019),
             aes(y = pages),
             shape = 1,
             #alpha = 2/3,
             position = pj,
