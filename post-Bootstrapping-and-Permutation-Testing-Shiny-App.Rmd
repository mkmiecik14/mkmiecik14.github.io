...
author: "Matthew J. Kmiecik & Ekarin Pongpipat"
date: "30 May 2018"
output:
  html_document:
    toc: yes
...
This post serves as a brief overview of the difference between boostrapping and permutation testing with a shiny app to visualize their differences. The example we use is correlation; however, these techniques can be extended to a variety of statistics (e.g., t-test, ANOVA, PCA). _Note: this post assumes you have preliminary knowledge of univariate statistics._
...
This process explained above is then repeated hundreds or thousands of iterations resulting in a distribution of values for your statistic of interest. This distibution will be centered about the original statistic value that you computed before any resampling occured. In other words, the mean of these stored values will equal your observed statistic.
...
A p-value is calculated by counting the number of statistic values that overlap with your observed statistic. Put another way, how many times did the statistic value that emerged from a "null distribution" overlap with your original computed statistic (before any shuffling). You take the number of times that the null distribution eclipsed your original value and divide it by the bumber of permutation iterations (number of observations in your null distribution). 

For example, let's say I ran permutation testing on a correaltion _r_ = .5 and shuffled X, kept Y, computed their correlation, stored this value, and repeated this 100 times. Out of 100 times, there were 4 correlations that emerged that were greater than .5. Therefore, my p-value for this correlation would be 4/100 = `r 4/100`.

## Shiny App
<hr >

Here's the shiny app used to illustrate these concepts. [Click here](https://mattkmiecik.shinyapps.io/boot-perm-app/) to see the app in full screen.

If the app does not appear, it may have reached the 25 active hours per month allotted to shinyapps.io free users (me!). In that case, see the code below to run it locally on your machine!
