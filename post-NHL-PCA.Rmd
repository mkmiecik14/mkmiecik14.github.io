...
To do this, let's first square the singular values (eigenvalues, $\lambda$) and sum them together. This is called __inertia__.
...
metric <- hawks_data_svd$v
rownames(metric) <- colnames(hawks_data_mat)

metric_fs <- as_tibble(hawks_data_svd$v) %>%
  mutate(Metric = colnames(hawks_data_mat))
  
ggplot(metric_fs, aes(V1, V2)) +
  geom_vline(xintercept = 0, alpha = 2/3) +
  geom_hline(yintercept = 0, alpha = 2/3) +
  geom_point() +
...
We can now take a look at how much each season (row) or metric (column) contributes to the new component or factor. To calculate the contribution of each factor, we square each factor score and divide it by the sum of squared factor, which is also the eigenvalue.
