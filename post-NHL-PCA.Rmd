...
contributions_years %>%
  ...
  ggplot(., aes(x = Season, y = Contributions)) +             
    ...
    geom_hline(data = contributions_years_means %>% filter(variable %in% c("Component_1", "Component_2")), aes(yintercept = mean), linetype = "dashed", alpha = 0.5) +  
     geom_hline(data = contributions_years_means %>% filter(variable %in% c("Component_1", "Component_2")), aes(yintercept = -mean), linetype = "dashed", alpha = 0.5) +
    facet_wrap(~ variable, labeller = as_labeller(c(`Component_1` = "Component 1\n", `Component_2` = "Component 2\n"))) +
    theme_minimal() +
...
Season 2008 and 2009 contribute more than the mean season on the negative side of component 1, while seasons 2016 to 2018 contribute more than the mean season on the positive side of component 1.

For component 2, only season 2013 contributes more than the mean season and on the negative side of component 2. 
...
contributions_metric %>%
  filter(factor %in% c("Component_1", "Component_2")) %>%
  ggplot(., aes(x = Metric, y = Contributions)) +
    geom_bar(stat = "identity") +
    geom_hline(data = contributions_metric_means %>% filter(factor %in% c("Component_1", "Component_2")), aes(yintercept = mean), linetype = "dashed", alpha = 0.5) +
    geom_hline(data = contributions_metric_means %>% filter(factor %in% c("Component_1", "Component_2")), aes(yintercept = -mean), linetype = "dashed", alpha = 0.5) +
    facet_wrap(~ factor, labeller = as_labeller(c(`Component_1` = "Component 1\n", `Component_2` = "Component 2\n"))) +
    theme_minimal() +
    theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  labs(caption = "\nNote: Dashed lines represent the mean contribution.")
```
